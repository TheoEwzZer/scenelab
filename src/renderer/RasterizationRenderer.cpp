...
RasterizationRenderer::RasterizationRenderer() {
    ...
    m_lightingShader.init("../assets/shaders/shader.vert", "../assets/shaders/lighting.frag");
    m_lightingShader.use();
    m_lightingShader.setVec3("lightColor", {1.0f, 1.0f, 1.0f});
    m_lightingShader.setVec3("lightPos", {2.f, 0.f, 0.0f});

    m_pointLightShader.init("../assets/shaders/shader.vert", "../assets/shaders/pointLight.frag");

    // Initialize view and projection matrices
    m_viewMatrix = glm::mat4(1.0f);
    m_viewMatrix = glm::translate(m_viewMatrix, glm::vec3(0.0f, 0.0f, -4.0f));
    m_projMatrix = glm::perspective(glm::radians(45.0f), 1920.0f / 1080.0f, 0.1f, 100.0f);
}
...
void RasterizationRenderer::updateTransform(int objectId, const glm::mat4& modelMatrix) {
...
void RasterizationRenderer::beginFrame() {
    ...
    for (size_t i = 0; i < m_renderObjects.size(); ++i) {
        ...
        if (obj.isLight) {
            m_lightingShader.setVec3("lightPos", glm::vec3(obj.modelMatrix[3]));
        }
    }
    ...
    for (size_t i = 0; i < m_renderObjects.size(); ++i) {
        ...
        if (obj.isActive) {
            ...
            if (error != GL_NO_ERROR && i == 0) {
                std::cout << "OpenGL error after drawing object " << i << ": " << error << std::endl;
            }
        }
    }
}
